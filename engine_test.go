...
func TestEmptyBoardAcceptsMoves(t *testing.T) {
  ...
  for row := range newBoard.Positions {
    for col := range newBoard.Positions[row] {
      if row != 1 && col != 1 {
        if newBoard.Positions[row][col] != EmptyPosition {
...
func TestCannotPlayOnOccupiedPosition(t *testing.T) {
  ...
  if err == nil {
    t.Errorf("GoGo should have prevented a move onto occupied position, but didn't return an error.")
  } else {
